...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            // Нажатие мышью (основная логика)
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
                sf::Vector2i mouse = sf::Mouse::getPosition(window);
                int paletteY = HEIGHT * PIXEL_SIZE + 50;
                
                // Если клик в пределах холста — рисуем
                ...
                // Если клик в пределах палитры — выбираем цвет
                else if (mouse.y >= paletteY && mouse.y <= paletteY + 30) {
            ...
            // Перемещение мыши (рисование кистью/ластиком)
        ...
        // Рисуем цветные прямоугольники палитры
